...
If so, then [migrating to Java 9](java-9-migration-guide) won't be straight-forward due to [split packages](https://blog.codefx.org/java/java-9-migration-guide/#Split-Packages).
Here's how to fix that problem.

### Overview

I use a little demo project that you can have a closer look at [over at GitHub](https://github.com/CodeFX-org/demo-java9-jsr305).
We're only concerned with the class `MixedJsr305AndJavaxAnnotation`, which uses both `@Nonnull` (from JSR 305) and `@Generated` (from the JDK).
...
First I want to [quote Stephen Connolly](https://blog.codefx.org/java/jsr-305-java-9/#comment-3582702088):
...
This is a Java EE module and since Oracle got rid of Java EE (now Enterprise Eclipse for Java, EE4J), it [does not resolve such modules by default](https://blog.codefx.org/java/java-9-migration-guide/#Dependencies-On-Java-EE-Modules).
You'll have to [use the `--add-modules` command line option](https://blog.codefx.org/java/five-command-line-options-to-hack-the-java-9-module-system/#Accessing-Internal-APIs-With--add-exports) to get it into the readability graph.
We'll see shortly how that affects your situation.

With all of that settled, let's see where you're at.

<contentimage slug="jsr-305"></contentimage>

## Non-modular Project {#nonmodularproject}

Assuming you are just trying to get your code to compile on Java 9 you will be in the situation that your project is non-modular, i.e.
it has no module declaration `module-info.java`.

### Missing Platform Module {#missingplatformmodule}
...
### Invisible JSR-305 Annotations {#invisiblejsr305annotations}
...
### Patching The Platform Module {#patchingtheplatformmodule}

The hammer that hits all the split-package-nails is [the `--patch-module` option](https://blog.codefx.org/java/five-command-line-options-to-hack-the-java-9-module-system/#Adding-Classes-To-Modules-With--patch-module).
...
### Keeping The Split On The Class Path {#keepingthesplitontheclasspath}
...
[jms_in\_action title="Learn how to fix these problems yourself:"]

## Modular Project {#modularproject}
...
the content of both `javax.annotation-api-1.3.1.jar` and `jsr305-3.0.2.jar`.
Taken together:
...
If you modularize your code, there is no way around `--patch-module` unless you want to create your own artifact combining the other two.
